...
/*   Updated: 2023/04/02 02:08:53 by absela           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_map_info(t_cub *data)
{
	...
	while (1)
	{
		if (data->info->flag_so == 1 && data->info->flag_we == 1 && data->info->flag_ea == 1 && data->info->flag_no == 1 && data->info->flag_f == 1 && data->info->flag_c == 1)
		{
			tmp = ft_strdup(check);
			free(data->map);
			data->map = tmp;
...
// void	store_colors(t_cub *data) // juste make it work with boths f and c
void	store_colors(int	*flor, char *str) // juste make it work with boths f and c
{
	char **tmp;
...
void	get_ply_info(t_cub *data)
{
	...
	while (data->map_final[i])
	{
		...
		{
			if ((data->map_final[i][j]) == 'N' || (data->map_final[i][j]) == 'E' || (data->map_final[i][j]) == 'W' || (data->map_final[i][j]) == 'S')
			{
				flag++;
...
void	re_allocate_map(t_cub *data)
{
	...
	while (data->map_final[i])
	{
		...
		ft_memset(data->map_final[i] + ft_strlen(tmp), ' ', data->map_lines - ft_strlen(tmp));
...
void	fill_map(t_cub *data)
{
	...
	while (i < data->map_lines - 1)
	{
		...
		{
			if (data->map_final[j][i] != '1' && data->map_final[j][i] != '0' && data->map_final[j][i] != 'P')
		...
		while (data->map_final[i][j])
		{
			if	(data->map_final[i][j] == '0' || data->map_final[i][j] == 'P')
			{
				if (data->map_final[i + 1] == NULL ||j < 1 || i < 1 || 
					data->map_final[i + 1][j] == ' ' ||
					data->map_final[i - 1][j] == ' '|| data->map_final[i][j - 1] == ' ' 
					|| data->map_final[i][j + 1] == ' ' || data->map_final[i][j + 1] == '\0')
				{
...
void	mutate_map(t_cub *data)
{
	...
	check_map(data);
	fill_map(data);
	// for (int k = 0; data->map_final[k]; k++)
	// {
	// 	printf("%s\n", data->map_final[k]);
	// }
}
...
void	parse_map(t_cub *data)
{
	...
	while ((data->map[i]) == '	' || data->map[i] == ' ' || (data->map[i]) == '\n')
		i++;
	while ((data->map[i]) == '1' || (data->map[i]) == '0' || (data->map[i]) == 'N'||
		(data->map[i]) == 'E' || (data->map[i]) == 'W' || (data->map[i]) == 'S' ||
		(data->map[i]) == '\n' || (data->map[i]) == ' ')
	{
		if (data->map[i] == '\n' && (data->map[i + 1] == '\n' || data->map[i + 1] == '\0'))
...
void	read_map(t_cub *data)
{
	...
	// printf("[%s]\n", data->info->f);
	// printf("\n******************************\n[%s]\n", data->map);
	// printf("floor color value [%d]\n", data->info->floor_color[0]);	
	// printf("floor color value [%d]\n", data->info->floor_color[1]);	
	// printf("floor color value [%d]\n", data->info->floor_color[2]);	
	// printf("floor color value [%d]\n", data->info->sky_color[0]);	
	// printf("floor color value [%d]\n", data->info->sky_color[1]);	
	// printf("floor color value [%d]\n", data->info->sky_color[2]);	
}
...
int	main(int ac, char **av)
{
	...
	if (ac != 2 || ft_strlen(av[1]) < 5 || ft_strncmp((av[1] + ft_strlen(av[1]) - 4), ".cub", 4))
